...
const { smsSend } = require("../helpers/helper");
...
//   });
  ...
  if(findUser.resetPasswordExpireTime > Data.now() || findUser.resetPasswordOTP == otp){
   apiResponse.sendSucess(res,401,"Time Expires and OTP invalid",null);
  }

  //   if (findUser.resetPasswordExpireTime > Data.now()) {

  //       const randomNumber = crypto.randomInt(100000, 999999);
  //       const expireTime = Data.now() + 10 * 60 * 60 * 1000;

  //     if (findUser.email) {
  //       //send email

  //       const verifyLink = `https://forn.com/verify/${findUser.email}`;
  //       const template = RegistrationTemplate(
  //         firstName,
  //         verifyLink,
  //         randomNumber,
  ...
  //   await findUser.save()
  //   apiResponse.sendSuccess(res,401,"Check Your email/phone",findUser)
  // }
  ...
  //     return res
  //       .status(301)
  //       .redirect(`www.fn.com/verify/${findUser.phoneNumber}`);
  //   } else {
  //     //again send email
  //     return res.status(301).redirect(`www.fn.com/verify/${findUser.email}`);
  //   }
...
exports.resetPassword = asynchandeler(async (req, res) => {
  ...
  if (newPassword !== confirmPassword) {
    throw new customError(
      401,
      "newPassword or confirm passowrd does not match",
    );
  }
